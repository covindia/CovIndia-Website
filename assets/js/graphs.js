...
$.when(
  $.ajax("https://v2.api.covindia.com/history-testing").then((response) => {
  ...
  tooltips: {
    ...
    custom: function (tooltipModel) {
      ...
      if (!tooltipEl) {
        ...
        tooltipEl.innerHTML =
          "<table style = 'background-color: rgba(0, 0, 0, 0.7); border-radius: 10px; padding: 5px;'></table>";
      ...
      if (tooltipModel.body) {
        ...
        bodyLines.forEach(function (body, i) {
          ...
          if (body[0].split(":")[0].trim() === "Negative") {
            count = tested.findIndex(
              (item) => item == body[0].split(":")[1].trim()
            );
            innerHtml +=
              "<tr><td>" +
              body +
              "</td></tr><tr><td>" +
              (100 - percentagePositive[count]) +
              "%" +
              "</td></tr>";
          } else if (body[0].split(":")[0].trim() === "Positive") {
            count = positive.findIndex(
              (item) => item == body[0].split(":")[1].trim()
            );
            innerHtml +=
              "<tr><td>" +
